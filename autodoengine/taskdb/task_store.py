...
def create_task(*, title: str, goal_text: str, current_node_uid: str, parent_task_uid: str | None = None) -> dict[str, Any]:
    ...
    with closing(_connect()) as connection, connection:
        connection.execute(
            """
            INSERT INTO "任务运行" (
                uid_任务, uid_工作流, "节点编码", "闸门编码", "动作决策", "运行状态", "工作区根路径",
                "输入摘要JSON", "输出摘要JSON", "开始时间", "结束时间", "操作人", "备注",
                "标题", "目标说明", "uid_当前节点", "uid_当前事务", "uid_父任务",
                "重试次数", "最大重试次数", "元数据JSON", "创建时间", "更新时间"
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                task_uid,
        ...
        row = connection.execute('SELECT * FROM "任务运行" WHERE uid_任务=?', (task_uid,)).fetchone()
    if row is None:
...
def update_task_status(task_uid: str, status: TaskStatus) -> None:
    ...
    with closing(_connect()) as connection, connection:
        cursor = connection.execute(
            'UPDATE "任务运行" SET "运行状态"=?, "更新时间"=?, "结束时间"=? WHERE uid_任务=?',
...
def update_task_metadata(task_uid: str, metadata: dict[str, Any], *, merge: bool = True) -> None:
    ...
    with closing(_connect()) as connection, connection:
        row = connection.execute('SELECT "元数据JSON" FROM "任务运行" WHERE uid_任务=?', (task_uid,)).fetchone()
        if row is None:
            raise KeyError(f"任务不存在：{task_uid}")
        ...
        connection.execute(
            'UPDATE "任务运行" SET "元数据JSON"=?, "更新时间"=? WHERE uid_任务=?',
